...
from rewave.environment import long_portfolio, env_specs
...
def test_src_outputs(spec_id):
    ...
    # np.testing.assert_almost_equal(1 / X0[:, -1, 0], 1 * y0[:])
    # on the other hand these should not
    try:
        np.testing.assert_almost_equal(1 / X0[:, -1, 0], 1 * y1[:])
...
def test_portfolio_env_hold(spec_id):
    ...
    assert df.portfolio_value.iloc[-1] > 0.9999, 'portfolio should retain value if holding cash'
    assert df.portfolio_value.iloc[-1] < 1.01, 'portfolio should retain value if holding cash'
...
def test_costs(spec_id):
    ...
    # keep the same portfolio should cost nothing
    obs, reward, done, info = env.step(np.array([0, 0, 1, 0]))
    np.testing.assert_almost_equal(
