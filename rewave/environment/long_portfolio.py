...
class DataSrc(object):
    ...
    def __init__(self, start_date, end_date,
                 ...
                 buffer_bias_ratio=1e-5, window_length=WINDOW_LENGTH, is_permed=False):
        """
        DataSrc.
        ...
        close_history = historyManager.historical_data(start=start_date, end=end_date, tickers=tickers_list,
                                                   features=['open'], adjusted=True)
        # don't consider the window_length
        close_history = close_history[window_length:]
        close_history = dataframe_fillna(close_history, "both")
        self.close_history = prepare_dataframe(close_history)
    ...
    def _step(self):
        ...
        # @TODO put in return
        last_close = self.close_history[:, self.step:self.step + self.window_length +1].copy()

        done = bool(self.step >= self.batch[-1])

        return X, y, last_w, setw, done
...
class PortfolioSim(object):
    ...
    def _step(self, w1, y1):
        ...
        p1 = p0 * mu * np.dot(y1, w0)  # (eq11) final portfolio value
...
class PortfolioEnv(gym.Env):
    ...
    def step(self, action):
        ...
        history, y1, last_w, setw, done1 = self.src._step()
