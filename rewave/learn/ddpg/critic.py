...
class CriticNetwork(object):
    ...
    def __init__(self, sess, root_net, inputs, state_dim, action_dim, learning_rate, tau):
        ...
        self.critic_model = Model(inputs=[inputs, self.action], outputs=self.out)

        #self.network_params = tf.trainable_variables()[num_actor_vars:]

        # Target Network
        self.target_action, self.target_out = self.create_critic_network(root_net=None)
        self.target_critic_model = Model(inputs=[self.target_inputs, self.target_action], outputs=self.target_out)
