...
from rewave.constants import *
from rewave.marketdata.quandlrequest import QuandlRequest
from rewave.tools.data import panel_fillna
...
class HistoryManager:
    ...
    # @TODO finish  - verify features names and take adjusted
    def historical_data(self, start, end, tickers=['AAPL','A'], features=['close'], adjusted=False):
        """
        construct a dataframe
        e.g
            ticker      AAPL                            A
            feature     open    high    low     close   open    high ...
        date
        2017-01-01      xxx     xxx     xxx     xxx     xxx     xxx ....

        data can be accessed by ticker from the column e.g. historical_dataFrame['AAPL']
        from the feature e.g. df.iloc[:, df.columns.get_level_values(1)=='open']
        from the date
        :param start:
        :param end:
        :param tickers:
        ...
        # convert to adjusted if needed
        features_list = features.copy()
        if adjusted:
            features_list = translate_to(source=features, dict=FEATURES_ADJUSTMENT)

        logging.debug("feature type list is %s" % str(features_list))
        ...
        try:
            for ticker in tickers:
                cursor = connection.cursor()
                sql = (
                "SELECT date,"+ ",".join(x for x in features_list) +"  from History WHERE ticker = \"{ticker}\" AND date >= \"{start}\" AND date<=\"{end}\" "
                ...
                # add header with ticker
                columns_header = pd.MultiIndex.from_product([[ticker], dataframe.columns.values],
                                                            names=['ticker', 'feature'])
                dataframe.columns = columns_header
                historical_dataFrame = pd.concat([historical_dataFrame, dataframe], axis=1)
        finally:
            connection.commit()
        # keep the order of columns
        if adjusted:
            historical_dataFrame = historical_dataFrame.rename(columns=REVERTED_FEATURES_ADJUSTMENT, level=1)
        historical_dataFrame = historical_dataFrame.reindex(columns=tickers, level=0)
        historical_dataFrame = historical_dataFrame.reindex(columns=features, level=1)
